port module Catalog.MiniApps.Text exposing (main)

import Browser
import Catalog.Components.Text as Text
...
type alias Model =
    { props : Result String Text.TextProps
    }
...
decodeProps : Value -> Result String Text.TextProps
decodeProps flags =
    case Decode.decodeValue (Decode.field "props" Decode.value) flags of
        Ok propsValue ->
            case Decode.decodeValue (Decode.dict Resolve.valueDecoder) propsValue of
                Ok resolvedDict ->
                    Text.propsDecoder resolvedDict
