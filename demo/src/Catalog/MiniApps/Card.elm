port module Catalog.MiniApps.Card exposing (main)

import Browser
import Catalog.Components.Card as Card
...
type alias Model =
    { props : Result String Card.CardProps
    }
...
decodeProps : Value -> Result String Card.CardProps
decodeProps flags =
    case Decode.decodeValue (Decode.field "props" Decode.value) flags of
        Ok propsValue ->
            case Decode.decodeValue (Decode.dict Resolve.valueDecoder) propsValue of
                Ok resolvedDict ->
                    Card.propsDecoder resolvedDict
