port module Catalog.MiniApps.Image exposing (main)

import Browser
import Catalog.Components.Image as Image
...
type alias Model =
    { props : Result String Image.ImageProps
    }
...
decodeProps : Value -> Result String Image.ImageProps
decodeProps flags =
    case Decode.decodeValue (Decode.field "props" Decode.value) flags of
        Ok propsValue ->
            case Decode.decodeValue (Decode.dict Resolve.valueDecoder) propsValue of
                Ok resolvedDict ->
                    Image.propsDecoder resolvedDict
