port module Catalog.MiniApps.Badge exposing (main)

import Browser
import Catalog.Components.Badge as Badge
...
type alias Model =
    { props : Result String Badge.BadgeProps
    }
...
decodeProps : Value -> Result String Badge.BadgeProps
decodeProps flags =
    case Decode.decodeValue (Decode.field "props" Decode.value) flags of
        Ok propsValue ->
            case Decode.decodeValue (Decode.dict Resolve.valueDecoder) propsValue of
                Ok resolvedDict ->
                    Badge.propsDecoder resolvedDict
