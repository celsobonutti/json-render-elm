module JsonRender.Internal.Condition exposing
    ( CompareValue(..)
    , Condition(..)
    , Operator(..)
    , RepeatContext
...
{-| Shared condition types and evaluation, used by both Visibility and $cond expressions.
...
sourceConditionDecoder : Decoder Condition
sourceConditionDecoder =
    Decode.oneOf
...
operatorDecoder : Source -> Decoder Condition
...
compareValueDecoder =
    Decode.oneOf
        ...
        , Decode.value |> Decode.map Literal
        ]


maybeNot : Condition -> Decoder Condition
maybeNot condition =
    Decode.oneOf
