...
truthinessSuite =
    describe "truthiness ($state only)"
        ...
        , test "returns false for missing path" <|
            \_ ->
                eval (Encode.object [ ( "other", Encode.bool True ) ])
                    Nothing
                    """{"$state": "/nonexistent"}"""
                    |> Expect.equal (Ok False)
        ]
...
negationSuite =
    describe "negation ($state + not)"
        [ test "returns false when state path is truthy" <|
            \_ ->
                eval (Encode.object [ ( "visible", Encode.bool True ) ])
                    Nothing
                    """{"$state": "/visible", "not": true}"""
                    |> Expect.equal (Ok False)
        , test "returns true when state path is falsy" <|
            \_ ->
                eval (Encode.object [ ( "visible", Encode.bool False ) ])
                    Nothing
                    """{"$state": "/visible", "not": true}"""
                    |> Expect.equal (Ok True)
        , test "not inverts an eq condition" <|
            \_ ->
                Expect.all
                    [ \_ ->
                        eval (Encode.object [ ( "tab", Encode.string "home" ) ])
                            Nothing
                            """{"$state": "/tab", "eq": "home", "not": true}"""
                            |> Expect.equal (Ok False)
                    , \_ ->
                        eval (Encode.object [ ( "tab", Encode.string "settings" ) ])
                            Nothing
                            """{"$state": "/tab", "eq": "home", "not": true}"""
                            |> Expect.equal (Ok True)
                    ]
        ...
        , test "not inverts a gt condition" <|
            \_ ->
                Expect.all
                    [ \_ ->
                        eval (Encode.object [ ( "count", Encode.int 10 ) ])
                            Nothing
                            """{"$state": "/count", "gt": 5, "not": true}"""
                            |> Expect.equal (Ok False)
                    , \_ ->
                        eval (Encode.object [ ( "count", Encode.int 3 ) ])
                            Nothing
                            """{"$state": "/count", "gt": 5, "not": true}"""
                            |> Expect.equal (Ok True)
                    ]
...
equalitySuite =
    describe "equality ($state + eq)"
        ...
        , test "supports state-to-state comparison" <|
            \_ ->
                eval (Encode.object [ ( "a", Encode.int 42 ), ( "b", Encode.int 42 ) ])
                    Nothing
                    """{"$state": "/a", "eq": {"$state": "/b"}}"""
                    |> Expect.equal (Ok True)
        , test "state-to-state comparison fails when different" <|
            \_ ->
                eval (Encode.object [ ( "a", Encode.int 1 ), ( "b", Encode.int 2 ) ])
                    Nothing
                    """{"$state": "/a", "eq": {"$state": "/b"}}"""
                    |> Expect.equal (Ok False)
        ]
...
numericSuite =
    describe "numeric comparisons"
        ...
        , test "gt: returns false when less" <|
            \_ ->
                eval (Encode.object [ ( "count", Encode.int 2 ) ])
                    Nothing
                    """{"$state": "/count", "gt": 3}"""
                    |> Expect.equal (Ok False)
        ...
        , test "gte: returns true when greater" <|
            \_ ->
                eval (Encode.object [ ( "count", Encode.int 6 ) ])
                    Nothing
                    """{"$state": "/count", "gte": 5}"""
                    |> Expect.equal (Ok True)
        , test "gte: returns false when less" <|
            \_ ->
                eval (Encode.object [ ( "count", Encode.int 4 ) ])
                    Nothing
                    """{"$state": "/count", "gte": 5}"""
                    |> Expect.equal (Ok False)
        ...
        , test "lt: returns false when greater" <|
            \_ ->
                eval (Encode.object [ ( "count", Encode.int 7 ) ])
                    Nothing
                    """{"$state": "/count", "lt": 5}"""
                    |> Expect.equal (Ok False)
...
dynamicRefSuite =
    describe "dynamic path references in comparison"
        [ test "eq with $state reference on right" <|
            \_ ->
                eval (Encode.object [ ( "count", Encode.int 5 ), ( "limit", Encode.int 5 ) ])
                    Nothing
                    """{"$state": "/count", "eq": {"$state": "/limit"}}"""
                    |> Expect.equal (Ok True)
        , test "lt with $state reference on right" <|
            \_ ->
                eval (Encode.object [ ( "count", Encode.int 3 ), ( "limit", Encode.int 5 ) ])
                    Nothing
...
implicitAndSuite =
    describe "array (implicit AND)"
        [ test "returns true when all conditions are true" <|
            \_ ->
                eval (Encode.object [ ( "isAdmin", Encode.bool True ), ( "tab", Encode.string "settings" ) ])
                    Nothing
                    """[{"$state": "/isAdmin"}, {"$state": "/tab", "eq": "settings"}]"""
                    |> Expect.equal (Ok True)
        , test "returns false when one condition is false" <|
            \_ ->
                eval (Encode.object [ ( "isAdmin", Encode.bool False ), ( "tab", Encode.string "settings" ) ])
                    Nothing
                    """[{"$state": "/isAdmin"}, {"$state": "/tab", "eq": "settings"}]"""
                    |> Expect.equal (Ok False)
        , test "returns false when all conditions are false" <|
            \_ ->
                eval (Encode.object [ ( "isAdmin", Encode.bool False ), ( "tab", Encode.string "home" ) ])
                    Nothing
                    """[{"$state": "/isAdmin"}, {"$state": "/tab", "eq": "settings"}]"""
                    |> Expect.equal (Ok False)
        ]
...
orSuite =
    describe "$or condition"
        [ test "returns true when at least one child is true" <|
            \_ ->
                eval (Encode.object [ ( "isAdmin", Encode.bool False ), ( "isModerator", Encode.bool True ) ])
                    Nothing
                    """{"$or": [{"$state": "/isAdmin"}, {"$state": "/isModerator"}]}"""
                    |> Expect.equal (Ok True)
        , test "returns true when all children are true" <|
            \_ ->
                eval (Encode.object [ ( "isAdmin", Encode.bool True ), ( "isModerator", Encode.bool True ) ])
                    Nothing
                    """{"$or": [{"$state": "/isAdmin"}, {"$state": "/isModerator"}]}"""
                    |> Expect.equal (Ok True)
        , test "returns false when all children are false" <|
            \_ ->
                eval (Encode.object [ ( "isAdmin", Encode.bool False ), ( "isModerator", Encode.bool False ) ])
                    Nothing
                    """{"$or": [{"$state": "/isAdmin"}, {"$state": "/isModerator"}]}"""
                    |> Expect.equal (Ok False)
        ...
        , test "supports booleans inside $or" <|
            \_ ->
                Expect.all
                    [ \_ ->
                        eval (Encode.object [ ( "ok", Encode.bool True ) ])
                            Nothing
                            """{"$or": [false, {"$state": "/ok"}]}"""
                            |> Expect.equal (Ok True)
                    , \_ ->
